pipeline {
    ...
    stages {
        ...
        stage('3. Code Quality Analysis') {
            steps {
                withSonarQubeEnv('SonarQubeServer') {
                    bat "${SCANNER_HOME}/bin/sonar-scanner -Dsonar.projectKey=spring-boot-app -Dsonar.sources=src/main/java -Dsonar.java.binaries=target/classes"
                }
            }
        }
        ...
        stage('4. Publish Artifact to Nexus') {
            steps {
                script {
                    writeFile file: 'settings.xml', text: """<settings><servers><server><id>nexus-credentials-id</id><username>admin</username><password>b91b081d-4ffb-4edf-874d-ac2ac05c2d7f</password></server></servers></settings>"""
